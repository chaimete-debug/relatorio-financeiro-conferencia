...
function renderRelatorio() {
  ...
  $('relatorioSaidasBody').innerHTML = despesas.length
    ? despesas.map(d => `<tr>
        ...
        <td>${escapeHtml(String(d.quantidade||''))}</td>
        <td class="num">${formatMoney(d.valorTotal)}</td>
      </tr>`).join('')
    : '<tr><td colspan="5" class="empty">Sem saídas.</td></tr>';
...
<body>
  ...
  <table>
    <thead><tr><th>Data</th><th>Categoria</th><th>Descrição</th><th>Taxa de Transf.</th><th style="text-align:right">Valor (MT)</th></tr></thead>
    <tbody>${linhasSaidas}</tbody>
    <tfoot><tr class="total"><td colspan="4">TOTAL SAÍDAS</td><td class="right">${fmt(r.despesasTotais)}</td></tr></tfoot>
  </table>
