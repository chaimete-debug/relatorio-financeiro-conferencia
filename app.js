// ── URL da API ──
const API_URL = 'https://script.google.com/macros/s/AKfycbzt1rab-mBQsht5qb9qLbWPdUZ-LEF20QYf_d0DgU2J1auDvOaJ-SoEqj8Hsyyo8oR9/exec';
...
function renderRelatorio() {
  ...
  $('relatorioSaidasBody').innerHTML = despesas.length
    ? despesas.map(d => `<tr>
        ...
        <td class="num">${d.taxaTransf ? formatMoney(d.taxaTransf) : ''}</td>
        <td class="num">${formatMoney(d.valorTotal)}</td>
      </tr>`).join('')
    : '<tr><td colspan="6" class="empty">Sem saídas.</td></tr>';
...
function exportarPDF() {
  ...
  const linhasSaidas = despesas.map(d => `
    <tr>
      ...
      <td>${escapeHtml(d.fornecedor||'')}</td>
      <td>${escapeHtml(d.descricao||'')}</td>
      <td class="right">${d.taxaTransf ? fmt(d.taxaTransf) : ''}</td>
      <td class="right">${fmt(d.valorTotal)}</td>
    </tr>`).join('');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:Arial,sans-serif;font-size:10pt;color:#1b2430;padding:1.5cm}
    h1{font-size:14pt;color:#163B63;text-align:center;text-transform:uppercase;margin-bottom:16pt}
    h2{font-size:11pt;margin:12pt 0 5pt}
    h2.entrada{color:#166534}
    h2.saida{color:#991B1B}
    table{border-collapse:collapse;width:100%;margin-bottom:12pt}
    th{background:#163B63;color:#fff;padding:5pt 7pt;text-align:left;font-size:9pt}
    td{padding:4pt 7pt;border-bottom:1pt solid #e7edf5;font-size:9pt;vertical-align:top}
    tr.total td{background:#EEF4FB;font-weight:bold;font-size:10pt}
    tr.saldo td{background:#163B63;color:#fff;font-weight:bold;font-size:12pt;padding:7pt 10pt}
...
<body>
  <h1>3ª Conferência da Mulher Nazarena<br>Relatório Consolidado</h1>

  <h2 class="entrada">Entradas</h2>
  <table>
    <thead><tr><th>Descrição</th><th class="right" style="text-align:right">Valor (MT)</th></tr></thead>
    <tbody>
  ...
  <table>
    <thead><tr><th>Data</th><th>Categoria</th><th>Beneficiários</th><th>Descrição</th><th style="text-align:right">Taxa Transf.</th><th style="text-align:right">Valor (MT)</th></tr></thead>
    <tbody>${linhasSaidas}</tbody>
    <tfoot><tr class="total"><td colspan="5">TOTAL SAÍDAS</td><td class="right">${fmt(r.despesasTotais)}</td></tr></tfoot>
  </table>
...
function exportarWord() {
  ...
  const linhasSaidas = despesas.map(d => `
    <tr>
      ...
      <td>${escapeHtml(d.fornecedor||'')}</td>
      <td>${escapeHtml(d.descricao||'')}</td>
      <td style="text-align:right">${d.taxaTransf ? fmt(d.taxaTransf) : ''}</td>
...
<body>
  ...
  <h2 class="saida">Saídas</h2>
  <table>
    <thead><tr><th>Data</th><th>Categoria</th><th>Beneficiários</th><th>Descrição</th><th style="text-align:right">Taxa de Transf.</th><th style="text-align:right">Valor (MT)</th></tr></thead>
    <tbody>${linhasSaidas}</tbody>
    <tfoot><tr class="total"><td colspan="5">TOTAL SAÍDAS</td><td class="right">${fmt(r.despesasTotais)}</td></tr></tfoot>
  </table>
